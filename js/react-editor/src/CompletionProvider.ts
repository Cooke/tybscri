...
export class TybscriCompletionItemProvider implements languages.CompletionItemProvider {
  ...
  async provideCompletionItems(
    ...
    const environment = getModelEnvironment(textModel);
...
function calcSymbolSuggestions(
  ...
  return currentNode.scope.getAll().map<monaco.languages.CompletionItem>((sug) => ({
    insertText: sug.name,
    kind:
...
function calcMemberSuggestions(
  ...
  const suggestions =
    members.map<monaco.languages.CompletionItem>((m) => ({
      ...
      range: wordRange,
      label: m.name,
      detail: widenType(type).displayName + "." + m.name + ": " + m.type.displayName,
    })) ?? [];
  return suggestions;
}

function calcRangeFromCurrentWord(textModel: monaco.editor.ITextModel, position: monaco.Position) {
